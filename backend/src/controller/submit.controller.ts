...
const loadSubmission = () => {
  ...
  const saveSubmission = (submissions: any) => {
...
export const submitController = asyncHandler(async (req, res) => {
  ...
  if (
    [name, email, phone, github_link, stopwatch_time].some(
      (field) => field.trim === ""
    )
  )
...
export const deleteSubmission = asyncHandler(async (req, res) => {
  ...
  export const updateSubmission = asyncHandler(async (req, res) => {
    ...
    return res
      .status(200)
      .json(
        new ApiResponse(200, 'Submission updated successfully', submissions[index])
      );
  });
  ...
  export const searchByEmail = asyncHandler(async (req, res) => {
    ...
    const submissions = loadSubmission();
    const foundSubmissions = submissions.filter(
